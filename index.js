...
class DragScroll {
  ...
  calculate() {
    this.progress = 0;
    console.log(this.items.length, this.items[0].clientWidth);
  ...
  raf() {
    ...
    this.items.forEach((item) => {
      item.style.transform = `scale(${1 - Math.abs(this.speed) * 0.01})`;
      item.querySelector("img").style.transform = `scalex(${
        1 + Math.abs(this.speed) * 0.01
      })`;
    });
  }
}
